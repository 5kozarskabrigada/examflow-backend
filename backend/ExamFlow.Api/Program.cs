...
using (var scope = app.Services.CreateScope())
{
    ...
    if (db.Database.ProviderName?.Contains("Npgsql", StringComparison.OrdinalIgnoreCase) == true)
    {
        ...
        await db.Database.ExecuteSqlRawAsync(@"
            ...
            ALTER TABLE ""MockExams"" ADD COLUMN IF NOT EXISTS ""CreatedByUserId"" INTEGER NULL;
            ALTER TABLE ""MockExams"" ALTER COLUMN ""CreatedByUserId"" DROP NOT NULL;
...
api.MapPatch("/mock-exams/{id:int}/lockdown-policy", async (int id, AppDbContext db, UpdateLockdownPolicyRequest request) =>
{
    var existing = await db.MockExams.FirstOrDefaultAsync(x => x.Id == id && x.DeletedAt == null);
    if (existing is null)
    {
    ...
    existing.LockdownPolicy = request.LockdownPolicy?.Trim() ?? "standard";
    await db.SaveChangesAsync();
    return Results.Ok(existing);
});
...
api.MapGet("/mock-exams/{examId:int}/sections", async (int examId, AppDbContext db) =>
{
    var sections = await db.ExamSections
        .Where(s => s.ExamId == examId)
        .OrderBy(s => s.ModuleType)
        .ThenBy(s => s.SectionOrder)
        .ToListAsync();

    return Results.Ok(sections);
});

api.MapPost("/mock-exams/{examId:int}/sections", async (int examId, AppDbContext db, ExamSection input) =>
{
    var exam = await db.MockExams.FirstOrDefaultAsync(x => x.Id == examId);
    if (exam is null)
    {
        return Results.NotFound(new { error = "Mock exam not found." });
    }
    ...
    input.ExamId = examId;
    input.CreatedAtUtc = DateTime.UtcNow;
    db.ExamSections.Add(input);
    await db.SaveChangesAsync();
    return Results.Ok(input);
});

api.MapPut("/mock-exams/{examId:int}/sections/{id:int}", async (int examId, int id, AppDbContext db, ExamSection input) =>
{
    var existing = await db.ExamSections.FirstOrDefaultAsync(x => x.Id == id && x.ExamId == examId);
    if (existing is null)
    {
        return Results.NotFound(new { error = "Section not found." });
    }

    existing.ModuleType = input.ModuleType;
...
api.MapDelete("/mock-exams/{examId:int}/sections/{id:int}", async (int examId, int id, AppDbContext db) =>
{
    var existing = await db.ExamSections.FirstOrDefaultAsync(x => x.Id == id && x.ExamId == examId);
    if (existing is null)
    {
    ...
    db.ExamSections.Remove(existing);
    await db.SaveChangesAsync();
    return Results.NoContent();
});
...
public record UpdateLockdownPolicyRequest(string LockdownPolicy);
